...
fn print_int(uv: vec2f, value: f32) -> f32 {
	var res = 0.0;
	let max_digits = 1.0 + ceil(log2(value) / log2(10.0));
	let digit_id = floor(uv.x);

	if digit_id > 0.0 && digit_id < max_digits {
        let digit_va: f32 = glsl_mod(floor(value / pow(10.0, max_digits - 1.0 - digit_id)), 10.0);
        res = sample_digit(i32(digit_va), vec2(fract(uv.x), uv.y));
	}
...
fn main(@builtin(position) position: vec4f) -> @location(0) vec4f {
    ...
    // Bottom: odd/even frame (mw.frame_number) box
